...
def clean_data(X_train):
    ...
    X_train = X_train.fillna(
        {
            "Demographic_Gender": X_train["Demographic_Gender"].mean(),
            "Symptom_Caugh": X_train["Symptom_Caugh"].mean(),
            "Symptom_Dyspnea": X_train["Symptom_Dyspnea"].mean(),
            "Symptom_Fever": X_train["Symptom_Fever"].mean(),
            "Symptom_Chiver": X_train["Symptom_Chiver"].mean(),
            "Symptom_Mylagia": X_train["Symptom_Mylagia"].mean(),
            "Symptom_Weakness": X_train["Symptom_Weakness"].mean(),
            "Symptom_LOC": X_train["Symptom_LOC"].mean(),
            "Symptom_Sore through": X_train["Symptom_Sore through"].mean(),
            "Symptom_Rhinorrhea": X_train["Symptom_Rhinorrhea"].mean(),
            "Symptom_Smelling disorder": X_train["Symptom_Smelling disorder"].mean(),
            "Symptom_nauseaVomit": X_train["Symptom_nauseaVomit"].mean(),
            "Symptom_Anorexia": X_train["Symptom_Anorexia"].mean(),
            "Symptom_Diarhhea": X_train["Symptom_Diarhhea"].mean(),
            "Symptom_ChestPain": X_train["Symptom_ChestPain"].mean(),
            "Symptom_Seizure": X_train["Symptom_Seizure"].mean(),
            "Symptom_SkinLesion": X_train["Symptom_SkinLesion"].mean(),
            "Symptom_Jointpain": X_train["Symptom_Jointpain"].mean(),
            "Symptom_Headache": X_train["Symptom_Headache"].mean(),
            "Symptom_AbdominalPain": X_train["Symptom_AbdominalPain"].mean(),
            "Symptom_Earpain": X_train["Symptom_Earpain"].mean(),
            "Symptom_Hemorrhasia": X_train["Symptom_Hemorrhasia"].mean(),
            "Symptom_Hemiparesia": X_train["Symptom_Hemiparesia"].mean(),
            "MH_PregnanAcy": X_train["MH_PregnanAcy"].mean(),
            "MH_CurremtSmoker": X_train["MH_CurremtSmoker"].mean(),
            "MH_Alcoholuser": X_train["MH_Alcoholuser"].mean(),
            "MH_Opiumuser": X_train["MH_Opiumuser"].mean(),
            "MH_Hookahuser": X_train["MH_Hookahuser"].mean(),
            "MH_HTN": X_train["MH_HTN"].mean(),
            "MH_IHD": X_train["MH_IHD"].mean(),
            "MH_CABG": X_train["MH_CABG"].mean(),
            "MH_CHF": X_train["MH_CHF"].mean(),
            "MH_Ashtma": X_train["MH_Ashtma"].mean(),
            "MH_COPD": X_train["MH_COPD"].mean(),
            "MH_DM": X_train["MH_DM"].mean(),
            "MH_Pneumonia": X_train["MH_Pneumonia"].mean(),
            "MH_CVA": X_train["MH_CVA"].mean(),
            "MH_GIdisorder": X_train["MH_GIdisorder"].mean(),
            "MH_CKD": X_train["MH_CKD"].mean(),
            "MH_RA": X_train["MH_RA"].mean(),
            "Cancer": X_train["Cancer"].mean(),
            "MH_HLP": X_train["MH_HLP"].mean(),
            "MH_Hep C": X_train["MH_Hep C"].mean(),
            "MH_Thyroid dysfunction": X_train["MH_Thyroid dysfunction"].mean(),
            "MH_Immunocompromised": X_train["MH_Immunocompromised"].mean(),
            "MH_ChronicSeizure": X_train["MH_ChronicSeizure"].mean(),
            "MH_TB": X_train["MH_TB"].mean(),
            "MH_Anemia": X_train["MH_Anemia"].mean(),
            "MH_Fattyliver": X_train["MH_Fattyliver"].mean(),
            "MH_Psychologicaldisorder": X_train["MH_Psychologicaldisorder"].mean(),
            "MH_Parkinson": X_train["MH_Parkinson"].mean(),
            "MH_Alzhimer": X_train["MH_Alzhimer"].mean(),
            "symtpm_to_referral": X_train["symtpm_to_referral"].mean(),
            "VS_O2satwithoutsupp": X_train["VS_O2satwithoutsupp"].mean(),
            "VS_PR": X_train["VS_PR"].mean(),
            "VS_diastolic BP": X_train["VS_diastolic BP"].mean(),
            "VS_Systolic BP": X_train["VS_Systolic BP"].mean(),
            "VS_RR": X_train["VS_RR"].mean(),
            "VS_T": X_train["VS_T"].mean(),
            "LAB_WBC_1": X_train["LAB_WBC_1"].mean(),
            "LAB_LYMPHH_1": X_train["LAB_LYMPHH_1"].mean(),
            "LAB_NEUT_1": X_train["LAB_NEUT_1"].mean(),
            "LAB_PLT_1": X_train["LAB_PLT_1"].mean(),
            "LAB_HB_1": X_train["LAB_HB_1"].mean(),
            "LAB_MCV_1": X_train["LAB_MCV_1"].mean(),
            "LAB_CR_1": X_train["LAB_CR_1"].mean(),
            "LAB_NA_First": X_train["LAB_NA_First"].mean(),
            "LAB_K_First": X_train["LAB_K_First"].mean(),
            "LAB_ALKP_First": X_train["LAB_ALKP_First"].mean(),
            "LAB_ESR_First": X_train["LAB_ESR_First"].mean(),
            "LAB_CPK_First": X_train["LAB_CPK_First"].mean(),
            "LAB_PTT_First": X_train["LAB_PTT_First"].mean(),
            "LAB_PT_First": X_train["LAB_PT_First"].mean(),
            "LAB_INR_First": X_train["LAB_INR_First"].mean(),
            "Demographic_Age": X_train["Demographic_Age"].mean(),
        }
    )
    return X_train
...
num = list(
    X_train.columns[
        X_train.columns.get_loc("VS_O2satwithoutsupp") : X_train.columns.get_loc(
            "Demographic_Age"
        )
        + 1
    ]
...
def lasso_feature_selector(X_train, y_train, X_test, X_ex):
    ...
    return (
        X_train_selected_Mortality_ICU,
        X_test_selected_Mortality_ICU,
        X_ex,
    ...
    for outcome, models in merged_dict.items():
        for model, metrics in models.items():
            ...
            (
                ...
                metrics[1]["y_pred_proba_test"],
                metrics[1]["y_pred_proba_external"],
            )
            data.append(
                [
                    outcome,
                    ...
                    y_pred_proba_test.tolist(),
                    y_pred_proba_external.tolist(),
                ]
            )
